...
from fungsi.query import insert_tweet_content, delete_tweet_content, create_tweet_content, create_table_alay, insert_alay_word, update_alay_word, delete_kalimat_alay, create_table_abusive # import query tweet_content
...
def text_processing_abusive_and_alay_file():
    ...
    combined_pattern = f"{karakter_khusus_regex}|{html_tag}|{non_latin_regex}|{whitespace_regex}|{alphanumeric_regex}"
    ...
    def count_abusive_words(text):
        hapus_abusive = "|".join(map(re.escape, list(kata_abusive)))
        words_removed = re.findall(hapus_abusive, text)
        for word in words_removed:
            if word in abusive_count:
                abusive_count[word] += 1
                sql.execute(update_abusive_word, (abusive_count[word], word))
            else:
                abusive_count[word] = 1
                sql.execute(insert_abusive_word,(word, abusive_count[word]))
    ...
    for index, text in enumerate(text_tweet):
        # abusive_count = count_abusive_words(text)
        cleaned_text2 = count_abusive_words(text)
        cleaned_text2 = tanda_baca.sub(r'\1', 
                        re.subn( combined_pattern, '',
                        non_latin_regex.sub('', 
                        kuote_belakang.sub('', 
                        pattern_alay.sub(lambda m: pengganti(m, jumlah_penggantian_kalimat), 
